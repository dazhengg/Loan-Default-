...
def classification(train,test,K):
	...
	X = train.values
	X_new = cla_feature_select(train)
...
def regression(train,test,clf_res,K):
	...
	# add some loss zero to the nonzero data set in order to improve the acuracy
	for i in range(100):
		if train['loss'][i] == 0:
			train_nonzero = train_nonzero.append(train.iloc[[i]])
